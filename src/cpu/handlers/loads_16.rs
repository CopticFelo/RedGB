...
pub fn load_r16_imm16(context: &mut Bus, opcode: u8) -> Result<String, GBError> {
...
pub fn load_r16mem_a(opcode: u8, context: &mut Bus) -> Result<String, GBError> {
...
pub fn load_a_r16mem(opcode: u8, context: &mut Bus) -> Result<String, GBError> {
    let param = R16::new(opcode, 4, R16Type::R16Mem)?;
    let addr = param.read(&context.registers);
...
pub fn ld_n16_sp(context: &mut Bus) -> Result<String, GBError> {
...
pub fn ld_hl_sp_delta(context: &mut Bus) -> Result<String, GBError> {
...
pub fn ld_n16_a(context: &mut Bus) -> Result<String, GBError> {
    let addr = alu::read_u16(&context.fetch(), &context.fetch());
    MemoryMap::write(context, addr, context.registers.a)?;
    Ok(format!("ld [{:#X}] a", addr))
}

pub fn ld_a_n16(context: &mut Bus) -> Result<String, GBError> {
    let addr = alu::read_u16(&context.fetch(), &context.fetch());
    context.registers.a = MemoryMap::read(context, addr)?;
    Ok(format!("ld a [{:#X}]", addr))
}

pub fn push(opcode: u8, context: &mut Bus) -> Result<String, GBError> {
...
pub fn pop(opcode: u8, context: &mut Bus) -> Result<String, GBError> {
    let r16_param = R16::new(opcode, 4, R16Type::R16Stk)?;
