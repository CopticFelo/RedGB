...
pub fn rotate_to_carry(opcode: u8, bus: &mut Bus) -> Result<String, GBError> {
    let is_left = alu::read_bits(opcode, 3, 1) == 0;
    let through_carry = alu::read_bits(opcode, 4, 1) == 1;
    let r8_param = R8::get_r8_param(false, opcode, 0, bus);
    let r8 = r8_param.read(bus)?;
    ...
    log += &r8_param.log();
    r8_param.write(bus, result)?;
    bus.registers
        .set_all_flags(&[(result == 0) as u8, 0, 0, carry as u8])?;
    Ok(log)
}

pub fn shift_to_carry(opcode: u8, bus: &mut Bus) -> Result<String, GBError> {
    let is_left = alu::read_bits(opcode, 3, 1) == 0;
...
pub fn swap(opcode: u8, bus: &mut Bus) -> Result<String, GBError> {
    let r8_param = R8::get_r8_param(false, opcode, 0, bus);
    let r8 = r8_param.read(bus)?;
...
pub fn test_bit(opcode: u8, bus: &mut Bus) -> Result<String, GBError> {
    let r8_param = R8::get_r8_param(false, opcode, 0, bus);
    let r8 = r8_param.read(bus)?;
...
pub fn reset_bit(opcode: u8, bus: &mut Bus) -> Result<String, GBError> {
    let r8_param = R8::get_r8_param(false, opcode, 0, bus);
    let r8 = r8_param.read(bus)?;
    let index = alu::read_bits(opcode, 3, 3);
...
pub fn set_bit(opcode: u8, bus: &mut Bus) -> Result<String, GBError> {
    let r8_param = R8::get_r8_param(false, opcode, 0, bus);
    let r8 = r8_param.read(bus)?;
    let index = alu::read_bits(opcode, 3, 3);
